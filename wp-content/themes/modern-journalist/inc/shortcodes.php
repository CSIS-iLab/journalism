...
//do simple weighted avarage
	...
	if ( $values['style'] == 'quote' ) {
		...
		$output .= '<div id="quote-header"><div class="post-header row full-width">';
		$output .= '<div id="header-quote-bg"><div id="header-bg" class="quote-trans" style="background-image:  url(\' ' . $image . ' \');">';
		$output .= '';
		$output .= '<div id="header-quote-fade" class="fade-in two '.$quotepos.'">';
		$output .= '<div id="header-quote-container">';
		$output .= '<div id="header-quote-title" >' . $quoteText . '</div>';
		...
		$output .= '<div class="post-intro">' . $values['intro'] . '</div>';
		$output .= '<div class="post-authors">By ' . $authors . '</div>';
		$output .= '<div class="img-desc"><span class="img-locator">PHOTO: </span>' . wp_get_attachment_caption( get_post_thumbnail_id( $postID ) );
		if ( $values['includesource'] == 'true' ) {
			if ( $values['sourceurl'] != '' ) {
