 ...
 */
$title = $link = $el_class = $el_id = $css = $css_animation = $el_width = $el_aspect = $align = $includesource = $sourcedesc = $sourceurl = $caption = '' ;
...
';
if ( !empty( $caption )){
$output .= '<div class="img-desc">'. $caption;
	}
if ( $includesource == 'true' ) {
    if ( !empty( $sourceurl )){
        $output .= '<div class="source-inline content-source"><a class="source-link" href="' . $sourceurl . '">' . $sourcedesc . '<i class="icon-external-open"></i></a></div>';
    } else {
        $output .= ' <div class="source-inline content-source">' . $sourcedesc . '</div>';
